<%
  ...
  str=Request.QueryString("ddcontenttype")  
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	<div class="header">
		...
		<div class="buttons">
				<a href="UserPage.asp" title="<%=Session("name")%>">My posts</a>&nbsp;
				<div class = "vertical"></div>&nbsp;
				<a href="Home.asp" style="color:white;text-shadow: 1px 1px 4px grey,1px 1px 4px white;">Home</a>&nbsp;
				<div class = "vertical"></div>&nbsp;
	...
	<div class="pages" style="position:static;">
		<table style="padding:30px;background-color:white;width: 60%">
			<form name="Filter" action="Home.asp?msg=selected" method="get">
			...
			<tr>
				<th style="text-align:left;color:#00B8FF">
					Hey, <%=Session("name")%>! What do you want to see?
				</th>
			</tr>
			<tr>
				<td>
	                <select name="ddcontenttype" style="width:absolute;">
	                	<%if str<>"" then %>
			...
			<tr>
				<td>
					<input style="margin-top:10px;" type="submit" value="Done">
				</td>
			</tr>
			</form>
		</table>
		...
		if str<>"" then
			rs.Open "SELECT * FROM tblposts where contenttype='"&str&"' ORDER BY postid desc", conn
		else
			rs.Open "SELECT * FROM tblposts ORDER BY postid desc", conn
		end if
		if rs.EOF<>True then
			do while not rs.EOF %>
				<div class="pages" style="position:static;">
